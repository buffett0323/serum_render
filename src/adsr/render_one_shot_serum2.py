...
def main(plugin_path, preset_dir, sample_rate=44100, bpm=120, 
    ...
    preset_paths = list(glob(str(Path(preset_dir) / '**' / '*.SerumPreset'), recursive=True)) #[:3]
    ...
    with open('../info/preset_to_id_serum2.json', 'w') as f:
        json.dump(preset_to_id, f, indent=4, ensure_ascii=False)
    with open('../info/id_to_preset_serum2.json', 'w') as f:
    ...
    for preset_path in tqdm(preset_paths, desc="Processing presets"):
        ...
        for note_name, note_number in c_notes.items():
            ...
            # Save audio file
            output_filename = f"Serum2_T{preset_id}_C{note_name}.wav"
