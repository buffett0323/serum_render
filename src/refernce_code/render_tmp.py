...
PLUGIN_NAME = "Serum" # "Serum 2 FX"
VSTPRESET_DIR = "../../python-vstpreset/converted"
...
def get_all_vstpresets(preset_dir):
    vstpreset_paths = []
    
    for p in os.listdir(preset_dir):
        if p.endswith('.vstpreset'):
            vstpreset_paths.append(os.path.join(preset_dir, p))
...
if __name__ == "__main__":
    # Get VST Presets
    vstpreset_paths = get_all_vstpresets(VSTPRESET_DIR)[:10] #[-2:]
    serum_dict = get_all_serum_states(vstpreset_paths)

    # Render Audio from Midi Files
    midi_file_path = "199311.mid"
    ...
    for preset_path, serum in serum_dict.items():
        ...
        output_filename = f"{preset_name}.{parsed_midi_file_name}.wav"
